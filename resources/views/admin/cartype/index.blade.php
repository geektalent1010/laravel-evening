              ...
              <table class="table datatables" id="myDataTable">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style="padding:0.2rem;">{{$data->maker}}</td>
                    <td style="padding:0.2rem;">{{$data->cartype}}</td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="varyModalLabel">車種マスタ</h5>
      ...
      <div class="modal-body">
        <form>
          <input type="hidden" id="upd_cartype_id">
          <div class="form-group">
            <label for="upd_maker" class="col-form-label">メーカー名:</label>
            <input type="text" class="form-control" id="upd_maker">
          </div>
          <div class="form-group">
            <label for="upd_cartype" class="col-form-label">車種名:</label>
            <input type="text" class="form-control" id="upd_cartype">
          </div>
        </form>
      </div>
...
<script type="text/javascript">
  ...
  function editData(record_id){
    ...
    $("#upd_cartype_id").val(data.id);
    $("#upd_maker").val(data.maker);
    $("#upd_cartype").val(data.cartype);
  }
  ...
  function deleteData(record_id){
    ...
    $.ajax({
        type: "POST",
        url: 'cartype/deletedata',
        data: {
            cartype_id : record_id,
        },
        dataType: 'json',
        success: function (data) {
          window.location.href = "{{url('/admin/cartype')}}";
        },
        error: function (data) {
            console.log(data);
        }
  ...
  function saveData(){
    var cartype_id = $("#upd_cartype_id").val();
    var maker = $("#upd_maker").val();
    var cartype = $("#upd_cartype").val();
